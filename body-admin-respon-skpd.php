...
switch ($_SESSION['ses_username']) {
	...
	case 'adminresponpln':
		$id_kategori = "k10";
		break;
	case 'adminresponbank':
		$id_kategori = "k11";
		break;
	case 'adminresponhotel':
		$id_kategori = "k12";
		break;
	case 'adminresponkuliner':
		$id_kategori = "k13";
		break;
	case 'adminresponpelabuhan':
		$id_kategori = "k14";
		break;
	case 'adminresponinstansi':
		$id_kategori = "k15";
		break;
	case 'adminresponsekolah':
		$id_kategori = "k16";
		break;
	case 'adminresponkesehatan':
		$id_kategori = "k17";
		break;
	default:
		$id_kategori = "";
...
$pN = $con->query("SELECT COUNT(*) jumlah FROM pengaduan WHERE id_kategori='$id_kategori' AND status_pengaduan ='N'")->fetch_assoc();
$pP = $con->query("SELECT COUNT(*) jumlah FROM pengaduan WHERE id_kategori='$id_kategori' AND status_pengaduan ='P'")->fetch_assoc();
$pY = $con->query("SELECT COUNT(*) jumlah FROM pengaduan WHERE id_kategori='$id_kategori' AND status_pengaduan ='Y'")->fetch_assoc();
$pT = $con->query("SELECT COUNT(*) jumlah FROM pengaduan WHERE id_kategori='$id_kategori'")->fetch_assoc();
			...
			<div class="w3-col m8 w3-container">
				...
				<table class="w3-table w3-card-2 w3-bordered w3-margin-bottom" style="background:rgba(0,100,255,.3)">
				    ...
				    if (!empty($_GET['status'])) {
				    	switch ($_GET['status']) {
					    	case 'Y':
					    		$q = $con->query("SELECT * FROM pengaduan WHERE id_kategori='$id_kategori' AND status_pengaduan='Y' ORDER BY id_pengaduan DESC");
					    		break;
					    	case 'P':
					    		$q = $con->query("SELECT * FROM pengaduan WHERE id_kategori='$id_kategori' AND status_pengaduan='P' ORDER BY id_pengaduan DESC");
					    		break;
					    	default:
					    		$q = $con->query("SELECT * FROM pengaduan WHERE id_kategori='$id_kategori' AND status_pengaduan='N' ORDER BY id_pengaduan DESC");
					    		break;
					    }
				    } else {
				    	$q = $con->query("SELECT * FROM pengaduan WHERE id_kategori='$id_kategori' ORDER BY id_pengaduan DESC");
				    }

...
$(document).ready(function(){
    $('#navbar').append(`<li class="w3-hide-small w3-right"><a href="logout.php" class="w3-hover-blue w3-tooltip"><i class="fa fa-sign-out"></i><span style="position:absolute;right:0px;top:0px" class="w3-border w3-border-blue w3-text w3-tag w3-small w3-round-large">Logout</span></a></li>
        <li class="w3-hide-small w3-right"><a href="?page=admin-respon" class="w3-hover-blue w3-tooltip"><i class="fa fa-list"></i><span class="w3-large"> Dashboard</span><span style="position:absolute;right:0px;top:0px" class="w3-hide w3-border w3-border-blue w3-text w3-tag w3-small w3-round-large">Dashboard</span></a></li>`);
});
</script>
...
function initialize(){
    ...
    function loadMarkerFirst(){
        $.getJSON(mainUrl+'load-data-pengaduan-map.php?lastId=100&startRow=0&limitRow=100&idDevice=777',function(result){
            $.each(result,function(i,data){
                if(data.posisi_lat_pengaduan>0 || data.posisi_lng_pengaduan>0){
                    getMarker(new google.maps.LatLng(data.posisi_lat_pengaduan,data.posisi_lng_pengaduan),
                        data.status_pengaduan+data.icon_kategori,
                        [data.id_kategori,data.id_sub_kategori,data.id_pengaduan],
                        data.kategori+" "+btnLihat(data.id_pengaduan),
                        createDescriptionInfoWindow(data.status_pengaduan+data.icon_kategori,
                            data.pengirim_pengaduan+'<br><span class="w3-text-black">Telp.'+data.telp_pengguna+'</span>',
                            mainUrl+'aw-uploads/profile/'+data.gambar_pengirim_pengaduan,
                            data.waktu_pengaduan,data.isi_pengaduan,
                            mainUrl+'aw-uploads/'+data.link_gambar+'compress700/'+data.gambar_pengaduan,
                            mainUrl+'aw-uploads/'+data.link_gambar+'compress700/'+data.gambar_pengaduan));
                }
            });
        }).complete(function(){
        ...
        $.getJSON(mainUrl+'load-data-pengaduan-baru.php',function(result){
            $.each(result,function(i,data){
                if(data.posisi_lat_pengaduan>0 || data.posisi_lng_pengaduan>0){
                    ...
                    if(result < 1){
                        getMarkerAnimation(new google.maps.LatLng(data.posisi_lat_pengaduan,data.posisi_lng_pengaduan),
                            data.status_pengaduan+data.icon_kategori,
                            [data.id_kategori,data.id_sub_kategori,data.id_pengaduan],
                            data.kategori+" "+btnLihat(data.id_pengaduan),
                            createDescriptionInfoWindow(data.status_pengaduan+data.icon_kategori,
...
function btnLihat(id) {
    var c = '<a href="?page=admin-respon&act=edit&id='+id+'" class="w3-button w3-round w3-blue" target="_blank"><i class="fa fa-search"></i></a>';
    return c;
}
...
<span id="search-btn" class="w3-tag w3-large w3-round-xlarge w3-green w3-hover-yellow" style="position:fixed;top:65px;left:265px;cursor:pointer"><b>==</b></span>
<div id="side-left" style="display:block;position:fixed;left:0px;top:52px;width:250px;overflow:auto;height:100%;background:rgba(0,100,255,0.7);">
    <ul id="filter-menu" class="w3-ul"></ul>
    <div style="height:52px"></div>
</div>

<div class="w3-row w3-card-4 w3-padding-tiny" style="position:fixed;bottom:12px;right:10px;width:400px;background:rgba(0, 77, 255, 0.5);">
